...
class WC_Payment_Gateway_Stripe_Afterpay extends WC_Payment_Gateway_Stripe_Local_Payment {
	...
	public function __construct() {
		...
		$this->countries          = $this->limited_countries = array( 'AU', 'CA', 'NZ', 'GB', 'US' );
	...
	public function get_local_payment_settings() {
		return wp_parse_args( array(
			...
			'payment_sections'            => array(
				...
				'description' => __( 'These are the additional sections where the Afterpay messaging will be enabled. You can control individual products via the Edit product page.', 'woo-stripe-payment' ),
			),
	...
	public function validate_local_payment_available( $currency, $billing_country, $total ) {
		...
		if ( $billing_country ) {
			...
			$_available = isset( $params[ $currency ] );

			// validate the amount is within the limits
			if ( $_available ) {
				list( $country, $min_amount, $max_amount ) = $params[ $currency ];
				$_available = $billing_country == $country && $min_amount <= $total && $total <= $max_amount;
			}
		}
	...
	public function get_localized_params( $context = 'checkout' ) {
		...
		$params['supported_locales'] = array( 'en-US', 'en-CA', 'en-AU', 'en-NZ', 'en-GB', 'fr-FR', 'it-IT', 'es-ES' );
